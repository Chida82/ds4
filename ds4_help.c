...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--ssd-streaming-mirror PATH", "Metal SSD streaming: byte-identical GGUF copy on a second SSD. Expert reads spill onto it in proportion to each disk's measured bandwidth (auto-calibrated). The engine auto-detects the regime at startup: if the model does not fit in available RAM it enables the mirror for prefill (the second disk cuts real disk waits); if the model fits in RAM it auto-disables the mirror (primary reads come from the page cache, so a slower disk would only hurt) -- override with DS4_METAL_STREAMING_MIRROR_FORCE=1. Decode reads stay on the primary by default (decode is often bind-bound, not disk-bound); set DS4_METAL_STREAMING_MIRROR_DECODE=1 to spill decode too when decode is the disk bottleneck. Verified at startup. Off by default.");
