...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--ssd-streaming-mirror PATH", "Metal SSD streaming: byte-identical GGUF copy on a second SSD. Prefill expert reads spill onto it in proportion to each disk's measured bandwidth (auto-calibrated); decode stays on the fast primary. Verified at startup. Gain scales with how disk-bound prefill is (a few percent when reads already overlap compute, more when the primary disk is the bottleneck); decode is unaffected. Off by default.");
